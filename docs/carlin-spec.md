...
**Escaping context.** Inside tags in the `:raw-text-tags` set (default
...
other dot blocks, normal escaping applies. The tag set is configuration, not a
language rule — set it to `#{}` when generating pure XML, extend it for custom
elements carrying code-like content.
...
Children under an `include` node are a positioned error (pug allows only filters
there; carlin expresses that as `include:filter`).
...
Text and escaped interpolation (`#{}`, `=`) HTML-escape. Raw positions — `!=`,
`!{}`, literal markup lines' static text, filter output, dot-block text under
`:raw-text-tags`, and `(raw …)` values anywhere including attribute position —
bypass escaping via the runtime **raw marker** (§6.1). One marker, every context.
...
- **Classes are additive across all sources, always** — concatenated in source
  order, collections flattened, map-conditional truthy keys included, duplicates
  preserved (browsers don't care; dedup can mask intent). Classes are outside the
  conflict policy: nothing declared is ever lost.
...
Text: `& < > " '` → entities. Attribute values: same, minus what the profile
proves unnecessary — pinned by the paranoid test file, not by prose.
...
cycle, dangling else clause) all report this way, as do reader errors, rebased
to template coordinates.
